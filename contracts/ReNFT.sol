...
contract RFT is ERC20 {
  ...
  // это функция предназначения для старта продажи НФТ
  // только владелец контракта может вызывать данную функцию
  function startResale() external {
       require(msg.sender == admin, 'only admin');
        nft.transferFrom(msg.sender, address(this), nftId);
        icoEnd = block.timestamp + 7 * 86400;
  }

  // Для покупки доли НФТ
